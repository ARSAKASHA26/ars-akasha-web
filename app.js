...
function getPlanDeliveryLabel(plan) {
  return isEbookPlan(plan) ? "E-book em PDF" : "Estudo entregue em PDF";
}
...
function getPlanGroupIntro(isEbook) {
  return isEbook
    ? "Produtos digitais em PDF, prontos para leitura, estudo pessoal e pratica espiritual."
    : "Leituras personalizadas entregues em PDF, preparadas a partir dos dados e caminhos espirituais do cliente.";
}

function getPaymentText() {
  return " Os dados do Pix serão enviados pelo atendimento via WhatsApp.";
}
...
function sendLeadToNetlify(leadData) {
  const formData = new FormData();
  formData.append("form-name", "leads-ars-akasha");
  formData.append("lead-id", leadData.id || "");
  formData.append("nome", leadData.name || "");
  formData.append("email", leadData.email || "");
  formData.append("whatsapp", leadData.whatsapp || "");
  formData.append("data-nascimento", leadData.birthDate || "");
  formData.append("hora-nascimento", leadData.birthTime || "");
  formData.append("cidade-nascimento", leadData.city || "");
  formData.append("consentimento-marketing", leadData.marketingConsent ? "Sim" : "Não");
  formData.append("titulo-leitura", leadData.readingTitle || "");
  formData.append("data-criacao", leadData.createdAt || "");

  fetch("/", {
    method: "POST",
    body: formData,
  })
    .then(() => console.log("Lead enviado para Netlify Forms"))
    .catch((error) => console.error("Erro ao enviar lead para Netlify:", error));
}
...
function renderPlans() {
  ...
  const entryOffers = sortPlansForSale(plans.filter((plan) => getPlanPriceValue(plan) <= 33.90));
  const entryOfferIds = new Set(entryOffers.map((plan) => plan.id));
  const studies = sortPlansForSale(plans.filter((plan) => !isEbookPlan(plan) && !entryOfferIds.has(plan.id)));
  const ebooks = sortPlansForSale(plans.filter((plan) => isEbookPlan(plan) && !entryOfferIds.has(plan.id)));
  const renderPlanCards = (items) => items.map((plan) => `
    <button class="plan-card ${plan.id === state.featuredPlanId ? "featured" : ""} ${state.selectedPlan === plan.id ? "active" : ""}" type="button" data-plan="${plan.id}">
      ...
      <span class="plan-delivery">${getPlanDeliveryLabel(plan)}</span>
      <p>${plan.summary}</p>
...
function renderCheckout() {
  ...
  const leadText = isEbookPlan(plan)
    ? (lead ? `${lead.name}, solicite este e-book em PDF pelo WhatsApp.` : "Gere sua leitura gratuita e escolha um e-book em PDF ou estudo para continuar.")
    : (lead ? `${lead.name}, continue este estudo em PDF pelo WhatsApp.` : "Gere sua leitura gratuita e escolha como deseja aprofundar.");
  ...
  els.checkoutPanel.innerHTML = `
    <div>
      ...
      <span>${leadText}</span>
      <small class="checkout-store">Os dados do Pix serão enviados pelo atendimento via WhatsApp</small>
    </div>
    <div class="checkout-actions">
...
function parseBirthDateInput(value) {
  ...
  return {
    ...
    display: `${padNumber(day)}/${padNumber(month)}/${year}`,
  };
}
...
function normalizeBirthTimeInput(value) {
  ...
  if (/^\d{1,2}:\d{2}$/.test(text)) {
    ...
    throw new Error("Hora de nascimento invalida.");
  }
...
function generateReading(data) {
  ...
  const birthTime = normalizeBirthTimeInput(data.birthTime || "12:00");
  const date = new Date(`${birthDate.iso}T${birthTime}`);
  const day = birthDate.day;
  const month = birthDate.month;
  const year = birthDate.year;
  const numerologyDate = birthDate.display;
  const sign = getZodiacSign(date);
  const element = getElementBySign(sign);
  const modality = getModalityBySign(sign);
  const nameNumber = getNameNumber(data.name);
  const lifePath = getLifePath(numerologyDate);
  const soulArcana = getSoulArcana(numerologyDate);
  const pathCard = getPathCard(numerologyDate);
  const journeyRune = getJourneyRune(numerologyDate);
  ...
  const hourWindow = getHourWindow(birthTime);
...
function renderReadingPlanOffers() {
  ...
  const renderOffers = (items) => items.map((plan) => `
    <div class="reading-plan-offer ${plan.id === state.featuredPlanId ? "featured" : ""} ${state.selectedPlan === plan.id ? "active" : ""}">
      ...
      <em>${plan.price}</em>
      <b class="plan-delivery">${getPlanDeliveryLabel(plan)}</b>
  ...
  return `
    <div class="reading-plan-group entry-group">
      <div class="reading-plan-heading">
        <strong>Mais acessiveis para comecar</strong>
        <small>Produtos de entrada para a primeira compra pelo WhatsApp.</small>
      </div>
      ${renderOffers(entryOffers)}
    </div>
...
function getOfferText(planId = state.selectedPlan) {
  ...
  if (!plan) {
    return `Ola, ${name}. Fiz minha leitura inicial no ARS AKASHA e quero continuar pelo WhatsApp para saber qual estudo combina melhor com meu momento.${paymentText}${contact}`;
  }

  const generatedDraw = formatDrawForWhatsApp(planId);
  if (isEbookPlan(plan)) {
    return `Ola, ${name}. Quero comprar o e-book em PDF ${plan.name} (${plan.price}) do ARS AKASHA. Conteudo: ${plan.features.join("; ")}.${paymentText}${contact}`;
  }

  return `Ola, ${name}. Quero o estudo em PDF ${plan.name} (${plan.price}) do ARS AKASHA. Inclui: ${plan.features.join("; ")}.${generatedDraw}${paymentText}${contact}`;
}
...
els.birthDate.addEventListener("input", (event) => {
  event.target.value = maskBirthDate(event.target.value);
});

els.birthTime.addEventListener("input", (event) => {
  event.target.value = maskBirthTime(event.target.value);
});
...
els.intakeForm.addEventListener("submit", (event) => {
  ...
  try {
    const birthDate = parseBirthDateInput(els.birthDate.value);
    const birthTime = normalizeBirthTimeInput(els.birthTime.value);
    els.birthDate.value = birthDate.display;
    els.birthTime.value = birthTime;
    ...
    // Envia os dados do lead para o Netlify Forms
    sendLeadToNetlify(state.currentLead);

    renderReading(reading, data);
    renderMetrics();
    renderCheckout();
  } catch (error) {
    console.error("Erro ao gerar leitura inicial", error);
    els.readingPanel.innerHTML = `
      <p class="eyebrow">Resultado</p>
      <h2>Nao consegui abrir a leitura agora</h2>
      <p class="detail-summary">Confira se todos os campos foram preenchidos. Use a data como DD/MM/AAAA e a hora como HH:MM. Se continuar, fale pelo WhatsApp para receber ajuda no atendimento.</p>
      <div class="offer-strip">
        <strong>Atendimento ARS AKASHA</strong>
        <span>Se preferir, envie seus dados pelo WhatsApp para receber orientacao.</span>
      </div>
      <a class="primary-button link-button" href="${getWhatsAppUrl()}" target="_blank" rel="noreferrer">Falar pelo WhatsApp</a>
    `;
  }
});
